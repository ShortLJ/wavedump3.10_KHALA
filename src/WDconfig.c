...
int ParseConfigFile(FILE *f_ini, WaveDumpConfig_t *WDcfg) 
{
	...
	// Output file name // Added by LJ
		if (strstr(str, "OUTPUT_FILE_NAME")!=NULL){
			read = fscanf(f_ini, "%s", str1);
			if (strcmp(str1, "DEFAULT")==0)
				WDcfg->UserSetNameFlag=0;
			if (!read){
				printf("Blank file name. Setting default\n");
				WDcfg->UserSetNameFlag=0;
			}
			else if(read && strcmp(str1, "DEFAULT")!=0){
				WDcfg->UserSetNameFlag=1;
				strcpy(WDcfg->UserSetName,str1);
			}
			continue;
		}

	// All data in one output file
		if (strstr(str,"OUTPUT_FILE_NUMBER")!=NULL){
			read = fscanf(f_ini, "%s", str1);
			if(strcmp(str1,"CHANNEL_BY_CHANNEL")==0)
				WDcfg->AllInOneFlag=0;
			else if(strcmp(str1,"ALL_IN_ONE")==0)
				WDcfg->AllInOneFlag=1;
			else 
				printf("%s: Invalid Parameter\n", str);
			continue;
		}
	// Pulse height threshold for zero suppresion
		if (strstr(str,"PULSE_HEIGHT_THRESHOLD")!=NULL){
			read = fscanf(f_ini, "%f", &WDcfg->PHthreshold);
			continue;
		}
