...
int WriteOutputFiles(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, void *Event)
{
    ...
    for (ch = 0; ch < WDcfg->Nch; ch++) {
		...
		if (!(WDcfg->UserSetNameFlag))          // Added by LJ
	                sprintf(fname, "%swave%02d.dat", path, ch);
		if (WDcfg->UserSetNameFlag)          // Added by LJ
			sprintf(fname, "%s%s%02d.dat", path, WDcfg->UserSetName, ch);          // Added by LJ
            ...
            if (!WDrun->fout[ch]) {
                char fname[100];
		if (!(WDcfg->UserSetNameFlag))
	                sprintf(fname, "%swave%02d.txt", path, ch);
		if (WDcfg->UserSetNameFlag)
			sprintf(fname, "%s%s%02d.txt", path, WDcfg->UserSetName, ch);
...
int WriteOutputFilesx742(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, CAEN_DGTZ_X742_EVENT_t *Event)
{
    ...
    for (gr=0;gr<(WDcfg->Nch/8);gr++) {
		...
		if(!WDrun->foutaio){
                        char fname[100];
	 		if (!(WDcfg->UserSetNameFlag))          // Added by LJ
				sprintf(fname, "%swave.dat", path);          // Added by LJ
			else if (WDcfg->UserSetNameFlag)          // Added by LJ
				sprintf(fname, "%s%s.dat", path, WDcfg->UserSetName);          // Added by LJ
			WDrun->foutaio = fopen(fname,"wb");
        	}
//if (ch==0 || ch==1){
/////////////zerosup
datamax=Event->DataGroup[gr].DataChannel[ch][0];
        ...
        if (Event->GrPresent[gr]) {
            for(ch=0; ch<9; ch++) {
                ...
                if( WDcfg->OutFileFlags& OFF_BINARY) {
	 			...
	 			if (!(WDcfg->UserSetNameFlag))          // Added by LJ
					sprintf(fname, "wave_%02d.dat", (gr*8)+ch);
				if (WDcfg->UserSetNameFlag)          // Added by LJ
					sprintf(fname, "%s_%02d.dat", WDcfg->UserSetName, (gr*9)+ch);          // Added by LJ
                            flag = 0;
                        }
 	 			...
 	 			if (!(WDcfg->UserSetNameFlag))		// Added by LJ
					sprintf(fname, "wave_%02d.txt", (gr*8)+ch);
				if (WDcfg->UserSetNameFlag)          // Added by LJ
					sprintf(fname, "%s_%02d.txt", WDcfg->UserSetName, (gr*8)+ch);          // Added by LJ
                            flag = 0;
                        }
